...
public class SpawnItemWorld : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.Space))
        {
            ...
            ItemWorld_Y item = Instantiate(itemWorld, newPos, Quaternion.identity).GetComponent<ItemWorld_Y>(); //{ item.id = 1; item.ammount = 1; }
            item.id = Random.Range(0, ItemDatabase_Y.Instance.itemDatas.Length);
